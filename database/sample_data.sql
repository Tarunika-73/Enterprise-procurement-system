...
VALUES
(1, 'LAP-001', 'ProBook 15', '15 inch laptop', 1, 1),
(2, 'LAP-002', 'AirBook 13', '13 inch light laptop', 1, 1),
(3, 'LAP-003', 'DevBook Pro', 'High perf laptop', 1, 1),

(4, 'DSK-001', 'WorkStation X', 'Standard desktop', 2, 1),
(5, 'DSK-002', 'PowerStation Y', 'High perf desktop', 2, 1),
(6, 'DSK-003', 'Mini PC', 'Compact desktop', 2, 1),

(7, 'FURN-001', 'Ergo Chair', 'Ergonomic chair', 3, 1),
(8, 'FURN-002', 'Standing Desk', 'Adjustable desk', 3, 1),
(9, 'FURN-003', 'Meeting Table', 'Large table', 3, 1),

(10, 'STAT-001', 'A4 Paper Ream', '500 sheets', 4, 1),
(11, 'STAT-002', 'Blue Pens (Box)', '50 pens', 4, 1),
(12, 'STAT-003', 'Stapler', 'Standard stapler', 4, 1),

(13, 'SOFT-001', 'OS Pro License', 'Operating System', 5, 1),
(14, 'SOFT-002', 'Office Suite', 'Productivity suite', 5, 1),
(15, 'SOFT-003', 'Antivirus', '1 year license', 5, 1),

(16, 'NET-001', 'WiFi Router', 'Dual band', 6, 1),
(17, 'NET-002', '24-port Switch', 'Gigabit switch', 6, 1),
(18, 'NET-003', 'Cat6 Cable 100m', 'Network cable', 6, 1),

(19, 'CLN-001', 'Glass Cleaner', '5L bottle', 7, 1),
(20, 'CLN-002', 'Floor Polish', '5L bottle', 7, 1),
(21, 'CLN-003', 'Paper Towels', '10 rolls', 7, 1),

(22, 'SNK-001', 'Coffee Beans', '1kg bag', 8, 1),
(23, 'SNK-002', 'Tea Bags', '100 pack', 8, 1),
(24, 'SNK-003', 'Mixed Nuts', '1kg pack', 8, 1),

(25, 'SEC-001', 'Dome Camera', '1080p security cam', 9, 1),
(26, 'SEC-002', 'Motion Sensor', 'PIR sensor', 9, 1),
(27, 'SEC-003', 'Access Card', 'RFID card', 9, 1),

(28, 'MKT-001', 'Rollup Banner', 'Standard size', 10, 1),
(29, 'MKT-002', 'Flyer A5', '1000 pack', 10, 1),
(30, 'MKT-003', 'Branded Pens', '500 pack', 10, 1);
...
SELECT 
    ...
    END,
    (MOD(n,5)+1),
    CONCAT('Need supplies for Q', MOD(n, 4) + 1), 'Approved', n * 100
...
SELECT 
    id, id, CONCAT('DN-', id), DATE_ADD(CURDATE(), INTERVAL 5 DAY), 'Delivered', 'FastShip', CONCAT('TRK', id, '999')
FROM purchase_orders;

-- 14. Receipts (20)
INSERT INTO receipts (id, delivery_id, received_by_user_id, receipt_date, condition_notes)
SELECT 
    id, id, 1, DATE_ADD(CURDATE(), INTERVAL 6 DAY), 'Received in good condition'
FROM deliveries;

-- 15. Invoices (20)
INSERT INTO invoices (id, invoice_number, receipt_id, vendor_id, invoice_date, due_date, total_amount, status)
SELECT 
    r.id, CONCAT('INV-', r.id), r.id, po.vendor_id, DATE_ADD(CURDATE(), INTERVAL 7 DAY), DATE_ADD(CURDATE(), INTERVAL 37 DAY), po.total_amount, 'Paid'
FROM receipts r
...
SELECT 
    id, id, CONCAT('PAY-', id), total_amount, DATE_ADD(CURDATE(), INTERVAL 15 DAY), 'Bank Transfer', 'Paid'
FROM invoices;
