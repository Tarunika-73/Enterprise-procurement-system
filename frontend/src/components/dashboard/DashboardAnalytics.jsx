...
const STATUS_COLORS = ['#7c6fd6', '#6eb5ff', '#198754', '#cc9a06', '#dc3545', '#6c757d', '#855bdf', '#0dcaf0'];
...
const DashboardAnalytics = () => {
  ...
  return <div className="analytics-dashboard">
    <ChartCard title="Purchase Request Status Distribution">{empty(status) || <ResponsiveContainer width="100%" height={260}><PieChart><Pie data={status} dataKey="count" nameKey="status" innerRadius={58} outerRadius={92} paddingAngle={2}>{status.map((entry, index) => <Cell key={entry.status} fill={STATUS_COLORS[index % STATUS_COLORS.length]} />)}</Pie><Tooltip formatter={(value) => [value, 'Requests']} /><Legend /></PieChart></ResponsiveContainer>}</ChartCard>
    <ChartCard title="Monthly Procurement Trend">{empty(trend) || <ResponsiveContainer width="100%" height={260}><LineChart data={trend} margin={{ top: 12, right: 12, left: -22, bottom: 0 }}><XAxis dataKey="month" tick={{ fontSize: 12 }} /><YAxis allowDecimals={false} tick={{ fontSize: 12 }} /><Tooltip formatter={(value) => [value, 'Requests']} /><Line type="monotone" dataKey="count" name="Requests" stroke="#7c6fd6" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} /></LineChart></ResponsiveContainer>}</ChartCard>
